...
//콘솔에서의 색상 개수
#define D_COLORNUM 13
//NPC블럭 생성 시 피봇 개수
#define D_PIVOTCNTOFNEWBLOCK 3
//NPC블럭생성 스피드 값
#define D_NEWBLOCKSPEED 0.1f
//콤보유효시간계산 스피드 값
#define D_NEWCOMBOSPEED 0.5f
//콤보유효시간 주기
...
//스페이스 키 입력이 연속으로 발생하지 않게 함
static int g_spaceKey;
...
void CleanMap()
{
	...
	for (y = 0; y < PULLBORAD; y++)
	{
		for (x = 0; x < PULLBORAD * 3; x++)
		{
...
void IncreaseBlockSpeed() {
	if (g_newBlockNum <= 0.5f)
		return;
	if( g_nScore / 300 != 0 && g_nScore % 300 == 0)
		g_newBlockNum -= 0.1f;
}
//게임의 연산 방식을 결정한다.
void GameOperation(char operation)
{
	...
	else if (g_nScore%300 >= 100) {
		g_Operation = 'x';
		DrawGameBoard();
	}
	else if (g_nScore%300 >= 0) {
		g_Operation = '-';
		DrawGameBoard();
	}
}
...
//모든 블록의 메모리를 해제한다.
void FreeRemoveAllBlock() {
	stMapBlock* pivot = g_mapBlockHead;
	stMapBlock* temp = NULL;
...
int DetectCollision(int posX, int posY) {
	...
	if (arrY >= GBOARD_HEIGHT) {
		g_IsCollision = true;
		g_newBlockCnt = (float)g_newBlockNum;
...
void AddBlockToBoard() {
	...
	if (g_newBlockCnt >= g_newBlockNum) {
		...
		while (nCount < D_PIVOTCNTOFNEWBLOCK) {
			...
			//다음 피봇과의 거리를 잰다. 0 1 2  D_PIVOTCNTOFNEWBLOCK :  3
			...
			newBlockColor = rand() % D_COLORNUM + 2;
			...
			for (int x = 0; x < temp->nWidth; x++) {
				gameBoardInfo[temp->nYPos][temp->nXPos + x] = temp->nValue;
			}
			nCount++;
		}
...
void DrawStartBoard()
{
	...
	SetCurrentCursorPos(curPos.X, curPos.Y + y + 3);
	Sleep(500);
	printf("  *충돌된 블록의 수가 다음 PC블록의 수가 됩니다.\n\n");
	Sleep(500);
	printf("  *주어지는 블록과 연산으로\n   일의 자리 숫자를 0으로 만들어보세요!\n\n");
	Sleep(500);
	printf("  - (뺄셈):  큰 수를 작은 수에 내려놓을 경우 ex) 3 - 5 -> 2\n\n");
	Sleep(500);
	printf("  + (덧셈):  10을 초과할 경우 일의 자리가 합이 됩니다. ex) 6 + 8 -> 4\n\n");
	Sleep(500);
	printf("  X (곱셈):  같은 수여도 블록이 사라집니다. ex) 7 * 7 -> 0\n\n");
	Sleep(500);
	printf("  ★ (아이템):  콤보가 3배수가 될 경우 생성됩니다. ex) 블록을 제거합니다.\n\n");
	Sleep(500);
...
int main()
{
	...
	PlaySound(TEXT("res/BackGroundMusic.wav"), NULL, SND_ASYNC | SND_LOOP);
